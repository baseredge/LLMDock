...
// converters (future llmdock) and the hosting application. Converters read
...
// (tests, external llmdock users) can use *Values.
...
// and llmdock deliberately does not use reflection to detect that case.
type Meta interface {
...
// hosts (the llmdock-native entry point).
type Values struct {
