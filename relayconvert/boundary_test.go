...
// boundary_test.go enforces the llmdock extraction dependency boundary
// (plans/llmdock-extraction-plan.md): packages that will move into the
// llmdock module must not grow imports of host-only packages. Entries in
...
// Packages (relative to the llmdock module root) covered by the boundary.
var kitDirs = []string{
	"relayconvert",
	"dto",
	"types",
...
// (everything outside llmdock/) and gin.
...
const kitModulePrefix = modulePrefix + "llmdock/"
		...
		err := filepath.WalkDir(filepath.Join(root, dir), func(path string, d os.DirEntry, err error) error {
			...
			for _, imp := range file.Imports {
				...
				for _, prefix := range forbiddenPrefixes {
					...
					t.Errorf("%s imports %q — forbidden inside future llmdock package %s (see plans/llmdock-extraction-plan.md)", rel, importPath, dir)
				}
			}
			return nil
		})
